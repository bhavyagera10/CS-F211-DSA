...
{
 int i,temp;
 for (i=sl.head ; st[i].next != -1 && i!=-1 ; i=st[i].next)
 {
...
void copy_sorted_ele(seqarr s , joblist ele)
{
 ...
 printf ("\n \n");
 for (i=2 ; i>=0 ; i--)
 { if (s[i].head!=-1) //for only valid priorities
 {
  for (j=st[s[i].head].next ; st[j].next != -1 &&s[i].head !=-1; j = st[j].next)
  {
   ele[index] = st[j].ele;
   //printjob(ele[index]);
   index++;
